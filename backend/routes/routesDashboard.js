...
        // --- CORREÇÃO DO DRIVER: Troca de execute() para query() ---
        const [mainResultRows, osResultRows] = await Promise.all([
            pool.query(mainQuery, paramsMain),
            pool.query(osQuery, paramsOS)
        ]);
...
        try {
            const [metaResultRows] = await pool.query(metaQuery); // Trocado para query()
...
        let conditions = ["UPPER(os.status) = 'CONCLUÍDO'"]; // 'status' minúsculo
        
        let params = [];
...
        const [rows] = await pool.query(sql, params); // Trocado para query()
        
...
    try {
        // --- CORREÇÃO DE LÓGICA: 'CONCLUÍDO' maiúsculo ---
...
        const [rows] = await pool.query(sql); // Trocado para query()
...
        if (agruparPor === 'dia') {
            // --- CORREÇÃO DE LÓGICA: 'CONCLUÍDO' maiúsculo e 'status' minúsculo ---
...
        const [[resumoRows]] = await connection.query(`
            SELECT 
...
        `); // Trocado para query() e corrigido 'CONCLUÍDO'
...
        const [anualRows] = await connection.query(sqlAnual); // Trocado para query()
...
        const [motoristaRows] = await connection.query(sqlMotorista); // Trocado para query()
...
        `;
        await pool.query(sql, [novaMeta, novaMeta]); // Trocado para query()
        
...
    // Esta rota não usa o banco de dados, então permanece igual
    const { query } = req.query;

    if (!query || query.length < 3) {
        return res.json([]);
    }
    // ... (código do axios permanece o mesmo) ...
    try {
...
            AS lucro;
        `;
        const [resultadoMesAnterior] = await pool.query(sqlLucroMesAnterior, [anoDoMesAnterior, mesAnterior, anoDoMesAnterior, mesAnterior]); // Trocado para query()

        const [despesasPassadas] = await pool.query(`
            SELECT SUM(valor) as totalDespesas, COUNT(DISTINCT MONTH(data)) as numMeses
            FROM financeiro
            WHERE YEAR(data) = ? AND MONTH(data) < ? AND tipo = 'Despesa'
        `, [anoAtual, mesAtual]); // Trocado para query()
...
        } else {
            const [despesasMesAtual] = await pool.query(`
...
        }
        
        let margemProjecao = 30; 
        try {
            const [configRows] = await pool.query("SELECT valor FROM configuracoes_sistema WHERE chave = 'margem_projecao'"); // Trocado para query()
...
    try {
        const [images] = await pool.query('SELECT id, image_url FROM slideshow_images ORDER BY data_criacao DESC'); // Trocado para query()
        
...
    try {
        const [[cliente]] = await pool.query( // Trocado para query()
...
        await pool.query("UPDATE configuracoes_sistema SET valor = ? WHERE chave = 'margem_projecao'", [margemProjecao]); // Trocado para query()
